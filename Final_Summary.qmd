...
author: "Aveline Mariya Shaji, Kyuri Kim, Satya Bharath Reddy Duvvi"
format:
  html:
    code-fold: true
...
Here is a SMART question for our project:
...
The dataset contains transactions made by credit cards in September 2013 by European cardholders and this dataset presents transactions that occurred in two days. Here we have 284,807 observations and 31 variables. 

It contains only numerical variables which are the result of a PCA transformation. Unfortunately, we were not given the original features and more background information about the data due to confidentiality issues. 
...
First, we checked for the empty cells, and it came out to be none. 
```{python}
CCFD_DATA.isnull().values.sum()
```

Next we checked for the duplicate cells. 
```{python}
CCFD_DATA.duplicated().sum()
```

We found 1081 duplicate cells so we removed them. 
```{python}
CCFD_DATA.drop_duplicates(keep=False,inplace=True)
```

Once again, we checked for a duplicate of cells and came out to be none. 
...
There are 460 fraudulent transactions out of 284,807 transactions in total. 
...
Looking at this pie chart, we can see that this dataset is highly imbalanced.
...
The first distribution plot is for 'Amount'. This one is strongly right skewed which probably means that the transaction amounts are mostly below $1000. We can see that even on the below summary, which shows $25,691 as the maximum. 
...
***Plot the distributions of every feature***

Here are distribution plots for all the features, red representing genuine transactions and blue representing fraudulent transactions. Here you can tell that for some features it shows the same pattern however some do not.  
...
Before proceeding with sampling techniques we implemented the modeling for Unbalanced Data to check how the Accuracy, Precision, Recall, and F1 Score will be.
...
Before proceeding with the modeling for Unbalanced Data we have to separate the original data frame into training and testing sets and separate the features from the labels.
...
We can observe that sampling is necessary to fit these models either with the data frames that were undersampled or oversampled (in order for our models to detect the patterns) and test it on the original testing set to improve Precision, Recall, and F1 Score.

## Modeling for Under Sampling Data

In this phase of the project, we will implement "Random Under Sampling" which basically consists of removing data in order to have a more balanced dataset and thus preventing our models from overfitting.
...
Now, we will implement a 'Random Over Sampling' technique to our dataset. This is basically adding more examples from the minority class, in this case, 'Fraud' transactions.
...
This is the correlation matrix of over-sampled dataset. This shows the same correlation to the under-sampled dataset. 

***Splitting the Data for Over Sampling***
Here, again, we split this over-sampled dataset into train (70%) and test (30%) dataset. 
```{python}
...
First, we created a model with random forest classifier. 
Here as you can see the accuracy, precision, and recall rate are all showing number close to or exactly 1, which means that this model is overfitted. 
...
Second, we built a model wth Logistic Regression. 
Here, the accuracy is 0.94, precision is 0.96, and recall is 0.92. This is still considered very high, eventhough the numbers imroved from the previous Random Forest model, this model is overfitted.
...
Third, we built a model with Decision tree classifier. 
Here, again, accuracy, precision, and recall rate is close to or exactly 1, which means that this is too overfitted. 
...
Lastly, we built a model using KNN classifier. 
...
Here, again, we have a very high accuracy, precision, and recall rate, which is 1. Thus, even this KNN gives us overfitted model.  
...
Overall, we conclude that over-sampled dataset gives us overfitted models. Thus, maybe using over-sampling is not suitable for this dataset. 
...
# References
Credit Card Fraud Detection. (n.d). Kaggle. Retrieved from https://www.kaggle.com/datasets/mlg-ulb/creditcardfraud

10 Techniques to deal with Imbalanced Classes in Machine Learning. (2020,July 23). Analytics Vidhya. Retrieved from https://www.analyticsvidhya.com/blog/2020/07/10-techniques-to-deal-with-class-imbalance-in-machine-learning/